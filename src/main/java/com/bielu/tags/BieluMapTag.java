 ...
 * @author  Guido Laures
 */
public class BieluMapTag extends CewolfTag {
		...
		try {
			...
			while (entities.hasNext()) {
				...
				if (!(ce instanceof LegendItemEntity)) {
					if (hasToolTips()) {
						writeOutToolTip(dataset, out, ce);
					}
					if (hasLinks()) {
	...
	private void writeOutToolTip(Dataset dataset, Writer out, ChartEntity ce) throws IOException, JspException {
		String toolTip = generateToolTip(dataset, ce);
		if (null != toolTip) {
			out.write("TITLE=\"" + toolTip + "\" ");
		}
	}
