 ...
 */
public class CountryInfoDao extends Dao<CountryInfo> implements ICountryInfoDao {

	/**
	 * @{inheritDoc}
	 */
	public CountryInfo findById(long id) {
		return (CountryInfo) getHibernateTemplate().get(CountryInfo.class, new Long(id));
	}
	...
	public List<CountryInfo> loadAll() {
		return getHibernateTemplate().find("from " + CountryInfo.class.getName() + " as ci order by ci.country");
	}
}
