...
/**
 * <code>IImageDao</code> TODO provide description
 *
 * @author Przemyslaw Bielicki
 */
public interface IImageDao extends IDao<Image>{
	
	List<Image> loadByRegion(String region);
	
	List<Image> loadByCountry(long countryId);
	
	List<Image> loadByDirectory(long directoryId);
	
	List<Image> loadMostPoplularImages(int limit);

	List<Image> loadLeastPoplularImages(int limit);
	
	double getCountSumByRegion(String region);
}
